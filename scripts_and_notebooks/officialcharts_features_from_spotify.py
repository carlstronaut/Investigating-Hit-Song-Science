...
officialcharts = pd.read_csv('updated_officialcharts.csv')
officialcharts_including_features = pd.DataFrame()
found = [False for row in officialcharts.iterrows()]
                ...
                try:
                    ...
                    if len(track["tracks"]["items"]) > 0:
                        ...
                        if (features):
                            ...
                            officialcharts_including_features = officialcharts_including_features.append(data, ignore_index=True)
                            found[index] = True
                            print('[%s %s%%] %s/%d found: "%s" by: %s' %
                                    (market, str((index*100//len(officialcharts.index))).rjust(3), str(len(officialcharts_including_features.index)).zfill(4), len(officialcharts.index), row['title'], row['artist']))
                    else:
                        print('[%s %s%%] Searching...' % (market,
                            str((index*100//len(officialcharts.index))).rjust(3)), end='\r')
                except ConnectionError:
                    continue
                break

officialcharts_including_features.to_csv('officialcharts_including_features_and_analysis.csv', index=False)
print('Done', ' ' * 40, '\n' * 2)
print(officialcharts_including_features.info())
